...
public final class TextTable {
  ...
  /**
   * Check, if cells are globally defined. Throws an exception, if it's not
   * the case.
   */
  ...
  /**
   * Check, if a default row format is defined. Throws an exception, if it's
   * not the case.
   */
   ...
   */
  public void hLine(final int width, final boolean marker, final boolean start,
          final boolean end) {
    if (start) {
      this.out.print("+");
    }
  ...
  /**
   * Same as {@link #cHeader(java.lang.String[], int[])}, but uses the
   * globally specified cells.
   *
   * @param columns Column titles
   */
  ...
  /**
   * Same as {@link #row(java.lang.Object[], java.lang.String[])}, but uses
   * the globally defined row format.
   *
